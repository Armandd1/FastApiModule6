...
# protected endpoint
@router.get("/secure", response_model=List[UserResponse], dependencies=[Depends(get_current_user)])
def get_users(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    users = db.query(User).all()
    return [UserResponse(username=u.username, email=u.email) for u in users]
...
@router.delete("/{id}", response_model=ResponseMessage, dependencies=[Depends(get_current_user)])
